...
{
  public sealed class HookInstallerUiHost : IInstallerUiHost
  {
    ...
    public void Run(IInstallerUiContext context, ManualResetEvent installStartEvent)
    {
      ...
      context.ConfigureSequenceHookAsyncInvoker(window.Dispatcher);
      HookInstallerUiSequenceHooks.Configure(context, window.ReportHookStatus);
      app.Run(window);
    }
